...
def resnet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt_dict = model_zoo.load_url(model_urls['resnet34'])
        net_dict = model.state_dict()
        pre_dict = {k: v for k, v in ckpt_dict.items() if k in net_dict}        
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ckpt_dict = model_zoo.load_url(model_urls['resnet50'])
        net_dict = model.state_dict()
        pre_dict = {k: v for k, v in ckpt_dict.items() if k in net_dict}        
        net_dict.update(pre_dict)
        model.load_state_dict(net_dict)
...
def resnet_small(pretrained=False, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [2, 3, 5, 2], **kwargs)
    if pretrained:
        ckpt_dict = model_zoo.load_url(model_urls['resnet50'])
        net_dict = model.state_dict()
        pre_dict = {k: v for k, v in ckpt_dict.items() if k in net_dict}        
